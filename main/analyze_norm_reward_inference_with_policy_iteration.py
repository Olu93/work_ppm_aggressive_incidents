...
import numpy as np
# %%
df_mean_results = pd.read_pickle('../data/experiment_inference_policy_iteration.pkl')
df_mean_results
# %%
...
for agent in ["PolicyIterationAgent", "QAgent", "SarsaAgent"]:
    for history in experiments:
        for rew_type in rew_types:
            ...
            with io.open(f'../data/po_res_analysis_transitions_{agent}_{rew_type}_{h}.txt', 'w') as f:
                for transition, cnt in tqdm(cnt_t.most_common()):
                    f.write(f"Count {cnt} | {transition}\n")
            with io.open(f'../data/po_res_analysis_epochs_{agent}_{rew_type}_{h}.txt', 'w') as f:
                for epoch_run, cnt in tqdm(cnt_x.most_common()):
                    f.write(f"Count {cnt} | {' -> '.join([str(x) for x in epoch_run])}\n")
            with io.open(f'../data/po_res_analysis_reactions_{agent}_{rew_type}_{h}.txt', 'w') as f:
